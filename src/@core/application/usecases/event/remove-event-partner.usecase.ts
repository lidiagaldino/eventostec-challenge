...
export class RemoveEventPartnerUsecase {
  constructor(
  ...
  async execute(dto: TEventPartnerInputDTO): Promise<void> {
    ...
    if (!eventPartner) throw new BadRequestException('this user is not a partner of this event')

    await this.eventRepository.removePartner(event, user)
    return
  }
}
